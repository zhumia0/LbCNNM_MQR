function [] = test_datasets(class)
close all;
disp('loading data ...')
data = loadmatfile(['./data/' class '_train.mat']);
GT = loadmatfile(['./data/' class '_test.mat']);
alpha = 0.05; %significance level
lambda = 20;
time_interval = 24; %time interval of m in MSIS
n = length(data);
disp([num2str(n) ' sequences.']);
global res; % length, training time, testing time, sMSIS (testing)
res = zeros(n,7);
